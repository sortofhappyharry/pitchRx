...
run_matrix<-ddply(abpitch_events,.(initial_state),summarize,mean(runs.roi, na.rm = TRUE))
...
batter_wOBAmax<-ddply(atbats,.(batter_name),summarize,wOBAmax = mean(event_value,na.rm=TRUE),wOBA2= (wOBAmax - avg_out_value)*LEAGUE_AVG_OBP/avg_woba,PA=sum(event_value,na.rm=TRUE)/wOBAmax)
